...
public static class MapGenerator
{
    ...
    /**
     * get postion where player born
     */ 
    public static Vector3 getBorn()
    {
        if (_firstLake == null) return new Vector3(1,1,1);
        return _firstLake.centerPoint;
    }
    ...
    private static MapGraph.MapNode _firstLake = null;
    ...
    private static void CreateLakes(MapGraph graph)
    {
        foreach (var node in graph.nodesByCenterPosition.Values)
        {
            ...
            {
                if(_firstLake==null) _firstLake = node;
                CreateLake(node);
            }
        }
    }
